...
#+caption: =html.sh=
#+begin_src shell :tangle html.sh :shebang #!/bin/sh :eval no
...
- =goaccess --log-format CLOUDFRONT --date-format CLOUDFRONT --time-format CLOUDFRONT --ignore-crawlers --ignore-status=301 --ignore-status=302 --keep-last=28 --output ~/stats/index.html= ::
  ...
  - =--output=~/stats/index.html= :: Output an HTML report to a file named =~/stats/index.html=.
...
#+caption: =csv.sh=
#+begin_src shell :tangle csv.sh :shebang #!/bin/sh :eval no
  find ~/stats/logs -name "*.gz" | \
      xargs gzcat | \
      /usr/local/bin/goaccess \
	  --log-format CLOUDFRONT \
